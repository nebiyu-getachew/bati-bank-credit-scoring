...
def plot_categorical_distributions(data, columns):
    ...
    categorical_cols = [col for col in columns if data[col].dtype == 'object']  # Filter only categorical columns
    n_cols = 2
...
def bivariate_analysis(data):
    ...
    sns.boxplot(x='ProductCategory', y='Amount', data=data)
    ...
    fraud_rate = data.groupby('ProductCategory')['FraudResult'].mean()
    plt.figure(figsize=(12, 6))
    ...
    channel_avg_amount = data.groupby('ChannelId')['Amount'].mean().sort_values(ascending=False)
    plt.figure(figsize=(10, 6))
    ...
    # 5. Customer Analysis
    customer_transactions = data.groupby('CustomerId')['Amount'].agg(['count', 'sum', 'mean'])
    ...
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x='TransactionStartTime', y='Amount', data=data)
...
def multivariate_analysis(data):
    ...
    fraud_by_category_channel = data.groupby(['ProductCategory', 'ChannelId'])['FraudResult'].mean().unstack()
    ...
    fraud_by_date = data.groupby('Date').agg({'FraudResult': 'mean', 'Amount': 'mean'})
...
def time_series_analysis(data):
    ...
    data['Date'] = data['TransactionStartTime'].dt.date
    daily_transactions = data.groupby('Date')['Amount'].sum().reset_index()
    ...
    data['DayOfWeek'] = data['TransactionStartTime'].dt.dayofweek
    ...
    # Trend Analysis
    monthly_transactions = data.groupby(data['TransactionStartTime'].dt.to_period('M')).agg({
        'TransactionId': 'count',
        'Amount': 'sum'
    })
    fig, ax1 = plt.subplots(figsize=(12, 6))
